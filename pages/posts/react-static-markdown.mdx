...
export const metadata = {
  ...
  description:
    "(Outdated) How I built the routes used by React Static for this website",
  date: new Date("2017-11-23T15:45"),
  edited: new Date("2018-05-20T11:48"),
  published: true
};

<PostHeader {...metadata} />

_Edit (20 May 2018): Updated to support React Static 5.0.0+_
...
async function makePage(root, filename) {
  ...
  if (isDir) {
    ...
    const layout =
      (data.layout && layoutMap[data.layout]) || layoutMap.ListPage;
...
<LargeLink url="/posts/react-in-markdown-updated/" title="React in Markdown" />
